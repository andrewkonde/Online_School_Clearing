...
//----------------------------------------------------------------------------------------------
$s="SELECT username FROM students WHERE username='$username'";
...
$sql="SELECT pass FROM students WHERE username='$username'";
...
$sql3="SELECT image FROM students WHERE username='$username'";
...
while($row= mysqli_fetch_array($result)){
	...
	//&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
	$sql4="SELECT schoolcode FROM registeredstudents WHERE username='$username'";
$result=(mysqli_query($connection,$sql4));
if($result->num_rows>0){
while($row= mysqli_fetch_array($result)){
	$myschoolcode=($row['schoolcode']);
	$_SESSION['myschoolcode']=$myschoolcode;
	//echo ('<img src="data:image/jpeg;base64,'.base64_encode($row['schoollogo']).'">');
}
}
...
while($row= mysqli_fetch_array($result)){
	...
	$_SESSION['myschoolcode']=$_SESSION['myschoolcode'];
	$_SESSION['myschoolname']=$_SESSION['myschoolname'];
	//$_SESSION['institution']=false;
	$_SESSION['membership']='student';
